...
import { Check, ArrowLeft, CheckCircle2, QrCode, Printer, UserPlus } from 'lucide-react';
    ...
    <div className="container py-8 max-w-3xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-sm font-medium text-green-600">Aplicación activa</span>
        </div>
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid grid-cols-4 mb-6">
              <TabsTrigger value="inicio" disabled={activeTab !== 'inicio' && !procesoCompletado}>Inicio</TabsTrigger>
              <TabsTrigger value="documentos" disabled={!clienteRegistrado && activeTab !== 'documentos'}>Documentos</TabsTrigger>
              <TabsTrigger value="pago" disabled={!documentoSeleccionado && activeTab !== 'pago'}>Pago</TabsTrigger>
              <TabsTrigger value="comprobante" disabled={!procesoCompletado && activeTab !== 'comprobante'}>Comprobante</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="inicio">
              <div className="space-y-4">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h2 className="text-lg font-medium">Registrar cliente nuevo</h2>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Input id="email" type="email" placeholder="Ej: juan@ejemplo.cl" />
                  </div>
                  <div>
                  ...
                  <Button className="w-full mt-4" onClick={handleRegistrarCliente}>
                    Registrar cliente
                  </Button>
                </div>
              </div>
                  ...
                  <div className="p-4 bg-gray-50 rounded-md mb-6">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-medium">
                        {documentosDisponibles.find(d => d.id === documentoSeleccionado)?.nombre}
                      </span>
                    </div>
                    <div className="flex justify-between mb-2">
                      <span className="text-sm text-gray-500">Precio:</span>
                      <span className="font-medium">
                        ${documentosDisponibles.find(d => d.id === documentoSeleccionado)?.precio}
                      </span>
                    </div>
                    <div className="flex justify-between mb-2">
                      <span className="text-sm text-gray-500">Su comisión:</span>
                      <span className="font-medium text-primary">
                        ${Math.round((documentosDisponibles.find(d => d.id === documentoSeleccionado)?.precio || 0) * 0.15)}
                      </span>
                    </div>
                  </div>
                )}
            ...
            <TabsContent value="comprobante">
              <div className="text-center space-y-6">
                ...
                <h2 className="text-xl font-medium">¡Pago procesado con éxito!</h2>
                
                <div className="bg-gray-50 p-6 rounded-md mx-auto max-w-sm">
                  <h3 className="font-medium text-lg mb-4">Comprobante de pago</h3>
                  <div className="space-y-2 text-left mb-4">
                    <div className="flex justify-between">
                      <span className="text-gray-500">Código:</span>
                      <span>VEC-{Math.floor(100000 + Math.random() * 900000)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Documento:</span>
                      <span>{documentosDisponibles.find(d => d.id === documentoSeleccionado)?.nombre}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Monto:</span>
                      <span>${documentosDisponibles.find(d => d.id === documentoSeleccionado)?.precio}</span>
                    </div>
                    <div className="flex justify-between">
                  ...
                  <div className="border-t pt-4 text-center">
                    <p className="text-sm text-gray-600 mb-1">El documento ha sido enviado automáticamente al correo del cliente.</p>
                    <p className="text-sm text-primary">Su comisión ha sido registrada automáticamente.</p>
                  </div>
                </div>
