...
`;
  ...
  fs.writeFileSync('docs/codigo_fuente_temp.md', markdown);
  
  // Configurar opciones del PDF
  const pdfOptions = {
    dest: 'docs/NotaryPro_Manual_Completo_y_Codigo_Fuente.pdf',
  ...
  // Generar PDF
  try {
    await mdToPdf({ path: 'docs/codigo_fuente_temp.md' }, pdfOptions);
    console.log('PDF generado con éxito: docs/NotaryPro_Manual_Completo_y_Codigo_Fuente.pdf');
    
    // Eliminar archivo temporal
    fs.unlinkSync('docs/codigo_fuente_temp.md');
