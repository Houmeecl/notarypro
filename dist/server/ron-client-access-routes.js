...
exports.ronClientAccessRouter = void 0;
const express_1 = __importDefault(require("express"));
const ron_client_code_generator_1 = require("./services/ron-client-code-generator");
const jwt_auth_service_1 = require("./services/jwt-auth-service");
const ronClientAccessRouter = express_1.default.Router();
exports.ronClientAccessRouter = ronClientAccessRouter;
/**
 * POST /api/ron-client/generate-access
 * Generar código de acceso para cliente RON
 */
ronClientAccessRouter.post('/generate-access', jwt_auth_service_1.authenticateJWT, (0, jwt_auth_service_1.requireRole)(['certifier', 'admin', 'notary']), async (req, res) => {
    try {
        ...
        const accessPackage = await (0, ron_client_code_generator_1.generateClientAccessPackage)(sessionId, Number(clientId), Number(documentId), certifierId, sessionType);
        res.status(201).json({
            success: true,
 ...
 */
ronClientAccessRouter.get('/access/:code', async (req, res) => {
    try {
        const accessCode = req.params.code;
        const clientIP = req.ip || req.connection.remoteAddress;
        const result = await (0, ron_client_code_generator_1.validateAndUseClientCode)(accessCode, clientIP);
        if (!result.success) {
 ...
 */
ronClientAccessRouter.get('/code-info/:code', async (req, res) => {
    try {
        const accessCode = req.params.code;
        const codeDetails = await (0, ron_client_code_generator_1.getClientCodeDetails)(accessCode);
 ...
 */
ronClientAccessRouter.get('/my-codes', jwt_auth_service_1.authenticateJWT, (0, jwt_auth_service_1.requireRole)(['certifier', 'admin', 'notary']), async (req, res) => {
    try {
        const certifierId = req.user?.userId;
        const { status = 'all', limit = 20 } = req.query;
        const codes = await (0, ron_client_code_generator_1.getCertifierClientCodes)(certifierId);
        ...
        const stats = await (0, ron_client_code_generator_1.getClientCodeStats)(certifierId);
        res.json({
            success: true,
            codes: limitedCodes,
            stats,
 ...
 */
ronClientAccessRouter.post('/send-access', jwt_auth_service_1.authenticateJWT, (0, jwt_auth_service_1.requireRole)(['certifier', 'admin', 'notary']), async (req, res) => {
    try {
        ...
        const codeInfo = (0, ron_client_code_generator_1.getClientCodeInfo)(accessCode);
        if (!codeInfo) {
 ...
 */
ronClientAccessRouter.post('/regenerate/:code', jwt_auth_service_1.authenticateJWT, (0, jwt_auth_service_1.requireRole)(['certifier', 'admin', 'notary']), async (req, res) => {
    try {
        const oldAccessCode = req.params.code;
        const { expirationHours = 24 } = req.body;
        const certifierId = req.user?.userId;
        const oldCode = (0, ron_client_code_generator_1.getClientCodeInfo)(oldAccessCode);
        if (!oldCode) {
            return res.status(404).json({
                success: false,
        ...
        const newAccessPackage = await (0, ron_client_code_generator_1.generateClientAccessPackage)(oldCode.sessionId, oldCode.clientId, oldCode.documentId, certifierId, oldCode.metadata.sessionType);
 ...
 */
ronClientAccessRouter.get('/stats', jwt_auth_service_1.authenticateJWT, (0, jwt_auth_service_1.requireRole)(['certifier', 'admin', 'notary']), async (req, res) => {
    try {
        const userId = req.user?.userId;
        const isAdmin = req.user?.role === 'admin';
        const stats = await (0, ron_client_code_generator_1.getClientCodeStats)(isAdmin ? undefined : userId);
        // Limpiar códigos expirados
        const cleanedCount = (0, ron_client_code_generator_1.cleanupExpiredClientCodes)();
        res.json({
            success: true,
            stats: {
                ...stats,
 ...
 */
ronClientAccessRouter.post('/cleanup', jwt_auth_service_1.authenticateJWT, (0, jwt_auth_service_1.requireRole)(['admin']), async (req, res) => {
    try {
        const cleanedCount = (0, ron_client_code_generator_1.cleanupExpiredClientCodes)();
        res.json({
            success: true,
 ...
 */
ronClientAccessRouter.get('/qr/:code', async (req, res) => {
    try {
        const accessCode = req.params.code;
