...
class SignatureVerificationService {
     ...
     */
    verifySignature(signatureBase64, metadata) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // 1. Realizar verificación local básica (siempre disponible)
                const localVerification = yield this.performLocalVerification(signatureBase64);
                // 2. Intentar verificación con API externa (si está disponible)
                let apiVerification = null;
                try {
                    apiVerification = yield this.callExternalVerificationAPI(signatureBase64, metadata);
                }
                catch (error) {
                ...
                if (finalResult.isValid) {
                    const storedSignature = yield this.storeSignature(signatureBase64, metadata, finalResult);
     ...
     */
    performLocalVerification(signatureBase64) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
     ...
     */
    callExternalVerificationAPI(signatureBase64, metadata) {
        return __awaiter(this, void 0, void 0, function* () {
     ...
     */
    storeSignature(signatureBase64, metadata, verificationResult) {
        return __awaiter(this, void 0, void 0, function* () {
            // Generar un ID único para la firma
            const signatureId = crypto.randomUUID();
            // Guardar la imagen en el sistema de archivos
            const signaturePath = yield this.saveSignatureToFile(signatureBase64, signatureId);
            ...
            const [signature] = yield db_1.db.insert(schema_1.signatures).values(newSignature).returning();
            return signature;
        });
    }
    /**
     ...
     */
    saveSignatureToFile(signatureBase64, signatureId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Eliminar el prefijo de data URI si existe
            const base64Data = signatureBase64.replace(/^data:image\/\w+;base64,/, '');
            const buffer = Buffer.from(base64Data, 'base64');
     ...
     */
    getStoredSignature(signatureId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const [signature] = yield db_1.db.select().from(schema_1.signatures).where((0, drizzle_orm_1.eq)(schema_1.signatures.id, signatureId));
                if (!signature || !signature.file_path) {
                    return null;
                }
                // Leer la imagen de firma
                if (fs.existsSync(signature.file_path)) {
                    const fileData = yield fs.promises.readFile(signature.file_path);
