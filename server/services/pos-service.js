...
class TransbankPOS {
    ...
    getTransactions(apiKey, storeId, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield axios_1.default.get(`https://api.transbank.cl/v1/pos/transactions`, {
                    headers: {
...
class FlowPOS {
    ...
    getTransactions(apiKey, storeId, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield axios_1.default.get(`https://api.flow.cl/v1/transactions`, {
                    headers: {
                        'flowApiKey': apiKey,
                        'Content-Type': 'application/json'
                    },
...
class KhipuPOS {
    ...
    getTransactions(apiKey, storeId, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield axios_1.default.get(`https://api.khipu.com/v1/payments`, {
                    headers: {
                        'Authorization': `Bearer ${apiKey}`,
                        'Content-Type': 'application/json'
                    },
...
class PosService {
    ...
    syncPartnerSales(partnerId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Get partner details
                const [partner] = yield db_1.db.select().from(schema_1.partners).where((0, drizzle_orm_1.eq)(schema_1.partners.id, partnerId));
                if (!partner) {
                ...
                const transactions = yield posProvider.getTransactions(partner.posApiKey, partner.posStoreId, startDate, endDate);
                if (transactions.length === 0) {
                    // Update last synced time even if no transactions
                    yield db_1.db.update(schema_1.partners)
                ...
                const transactionsToInsert = transactions.map(transaction => (Object.assign(Object.assign({}, transaction), { partnerId,
                    commissionRate, commissionAmount: Math.round(transaction.amount * commissionRate), synchronized: true, createdAt: new Date() })));
                // Insert transactions to database
                yield db_1.db.insert(schema_1.posTransactions).values(transactionsToInsert);
    ...
    getAvailableProviders() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield db_1.db.select().from(schema_1.posProviders).where((0, drizzle_orm_1.eq)(schema_1.posProviders.isActive, true));
        });
    }
    // Get transactions for a partner in a date range
    getPartnerTransactions(partnerId, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return yield query.orderBy(schema_1.posTransactions.transactionDate);
        });
    }
    // Configure POS integration for a partner
    configurePosIntegration(partnerId, posProviderName, posApiKey, posStoreId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Check if the provider exists and is active
            const [provider] = yield db_1.db.select().from(schema_1.posProviders).where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.posProviders.name, posProviderName), (0, drizzle_orm_1.eq)(schema_1.posProviders.isActive, true)));
            if (!provider) {
    ...
    getPartnerSalesSummary(partnerId, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get transactions
            const transactions = yield this.getPartnerTransactions(partnerId, startDate, endDate);
            if (transactions.length === 0) {
                return {
                    totalTransactions: 0,
                    totalAmount: 0,
